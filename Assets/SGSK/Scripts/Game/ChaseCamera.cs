...
public class ChaseCamera : MonoBehaviour
{
	...
	public float desiredDistance		= 100f;		// Distance from the target that the camera tries to maintain
	public float followRotationSpeed	= 1000f;		// The higher the value the faster the camera adjusts to rotation changes
	public float followVelocitySpeed	= 1000f;		// The higher the value the faster the camera adjusts to velocity changes
	public float followVelocityLag		= 0f;	// The higher the value the more the camera will lag behind the target (0 for no lag)
	...
	void UpdateTransform (float delta)
	{
		...
		mCurrentRot = mTargetRot;//Quaternion.Slerp(mCurrentRot, mTargetRot, Mathf.Clamp01(delta * followRotationSpeed));
		
		// Camera's position should always be behind the target
		Vector3 camPos = mCurrentPos - (mCurrentRot * mBaseRot) * Vector3.forward * 1.2f;// * desiredDistance;
		...
		mTrans.rotation = mCurrentRot;//Quaternion.LookRotation(mCurrentPos - camPos + mCurrentRot * mFarTarget, mCurrentRot * Vector3.up);
